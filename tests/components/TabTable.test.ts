...
const ROWS: TabRow[] = [
  { tabId: 1, title: 'Alpha', domain: 'a.com', seconds: 100, lastActiveAt: 3000 },
  { tabId: 2, title: 'Beta', domain: 'b.com', seconds: 300, lastActiveAt: 1000 },
  { tabId: 3, title: 'Gamma', domain: 'c.com', seconds: 200, lastActiveAt: 2000 },
];
...
describe('TabTable', () => {
  test('defaults to sorting by tracked time, descending', () => {
    ...
    expect(trackedOrder(w)).toEqual(['Beta', 'Gamma', 'Alpha']); // 300,200,100
  ...
  test('clicking the Tracked header toggles to ascending', async () => {
    ...
    expect(trackedOrder(w)).toEqual(['Alpha', 'Gamma', 'Beta']); // 100,200,300
  ...
  test('switching to Last active sorts by recency and updates aria-sort', async () => {
    ...
    expect(trackedOrder(w)).toEqual(['Alpha', 'Gamma', 'Beta']); // lastActive 3000,2000,1000
