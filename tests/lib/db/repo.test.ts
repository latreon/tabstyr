...
import type { DailyStat, MonthlyStat, Session, TabMeta } from '@/lib/types';
...
// --- seeding through the shipped write paths -------------------------------
// The repo exposes exactly the writers the extension uses. Tests seed through
// those rather than through test-only primitives, so what they exercise is what
// ships (an earlier version of this file drove several helpers that no longer had
// any production caller).
const seedSessions = (sessions: Session[]) => repo.commitSessions(sessions, []);
const seedStats = (deltas: DailyStat[]) => repo.commitSessions([], deltas);
/** Seed the monthly archive. restoreAll is its only direct writer (prune is the
 *  other, indirect one) — note it CLEARS every store first, so seed monthly first. */
const seedMonthly = (rows: MonthlyStat[]) => repo.restoreAll([], [], [], rows);

/** Foreground seconds per stable tab key — the attribution `tabKey` exists for. */
...
describe('sessions + stats', () => {
  test('commitSessions writes sessions and stats in one transaction', async () => {
    ...
    expect(await secondsByKey('k1')).toBe(60);
    expect(await secondsByKey('k2')).toBe(60);
  ...
  test('commitSessions accumulates stat deltas into existing rows', async () => {
    await seedStats([{ date: '2026-06-11', domain: 'a.com', seconds: 60, audioSeconds: 0 }]);
  ...
  test('sessions keep separate totals when a restart reuses a tab id', async () => {
    // Same numeric tabId, different stable keys — the totals must not merge.
    await seedSessions([session({ tabId: 7, tabKey: 'old' }), session({ tabId: 7, tabKey: 'new' })]);
    expect(await secondsByKey('old')).toBe(60);
    expect(await secondsByKey('new')).toBe(60);
  });

  test('background-audio sessions are stored but are not foreground time', async () => {
    await seedSessions([
      session({}), // 60s foreground
      session({ start: T0 + 100_000, end: T0 + 130_000, audio: true }), // 30s audio
    ]);
    expect(await secondsByKey('k1')).toBe(60);
  ...
  test('applyDailyStatsMax takes the max per row and is idempotent', async () => {
    await seedStats([{ date: '2026-06-11', domain: 'a.com', seconds: 100, audioSeconds: 20 }]);
...
describe('backfillKeylessSessions', () => {
  ...
  test('stamps only keyless sessions whose tabId matches a current tab', async () => {
    ...
    expect(await secondsByKey('k1')).toBe(120);
  });

  test('is a no-op for an empty map', async () => {
    await seedSessions([keyless({ tabId: 1 })]);
...
describe('prune + wipe', () => {
  test('pruneBefore keeps records exactly at the cutoff', async () => {
    await seedSessions([session({ start: T0, end: T0 + 60_000 })]);
    await seedStats([{ date: '2026-03-13', domain: 'a.com', seconds: 10, audioSeconds: 0 }]);
  ...
  test('pruneBefore removes old sessions and old daily stats', async () => {
    await seedSessions([session({ start: T0 - 100 * DAY, end: T0 - 100 * DAY + 60_000 }), session({})]);
    await seedStats([
...
describe('monthly rollup archive', () => {
  ...
  test('re-running pruneBefore does not double-count into the archive (idempotent)', async () => {
    await seedStats([{ date: '2026-01-05', domain: 'a.com', seconds: 100, audioSeconds: 0 }]);
    await repo.pruneBefore('2026-03-13', T0 - 90 * DAY, '2000-01');
  ...
  test('successive prunes accumulate into the same month bucket', async () => {
    ...
    await seedStats([{ date: '2026-01-09', domain: 'a.com', seconds: 40, audioSeconds: 0 }]);
  ...
  test('pruneBefore keeps a monthly archive row exactly at monthlyCutoff', async () => {
    await seedMonthly([{ month: '2022-01', domain: 'a.com', seconds: 30, audioSeconds: 0 }]);
    await repo.pruneBefore('2026-03-13', T0 - 90 * DAY, '2022-01');
    expect(await repo.getAllMonthlyStats()).toEqual([
  ...
  test('a freshly archived row older than monthlyCutoff is pruned in the same run', async () => {
    ...
    await seedStats([{ date: '2020-01-05', domain: 'a.com', seconds: 100, audioSeconds: 0 }]);
    await repo.pruneBefore('2026-03-13', T0 - 90 * DAY, '2022-01');
    expect(await repo.getAllMonthlyStats()).toEqual([]);
  });
});
