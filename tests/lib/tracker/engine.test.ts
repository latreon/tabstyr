...
describe('TrackerEngine boundary safety', () => {
  ...
  test('handleFocus never opens a session for an excluded domain', () => {
    const e = new TrackerEngine();
    expect(e.handleFocus(1, 'https://reddit.com/r/all', T0, false, true)).toEqual([]);
    expect(e.getState().focused).toBeNull();
    // focusing a tracked page then an excluded one closes the tracked session
    // and starts none, exactly like navigating to an internal page.
    e.handleFocus(2, 'https://a.com', T0);
  ...
  test('handleUrlChange closes the session (and drops background audio) on navigating to an excluded domain', () => {
    const e = new TrackerEngine();
    e.handleFocus(1, 'https://a.com', T0);
    const closed = e.handleUrlChange(1, 'https://reddit.com/r/all', T0 + 60_000, true);
    expect(closed).toHaveLength(1);
    expect(closed[0].domain).toBe('a.com');
    expect(e.getState().focused).toBeNull();
  });
